...
## ⚡ 2026-07-15 勘误（静怡口述更正，以下四条以此为准）

1. **① 计费核查——解除**。静怡确认：`-p` 在 6/15 按量新规之后**又宣布回归订阅**了。教程的计费论断已过时，本机桥照旧免费，不用查了，「总闸」撤销。
2. **「补思考过程显示」——翻案，升级为下一步头号需求**。原否决依据「39 个桥会话 thinking 全空」归因错了：那些会话跑的都是不出思考正文的模型（settings.json 默认 opus-4-7 等）。静怡明确需要看思考过程。实施走核验员修正过的路线：`--output-format stream-json` 直读结构化事件（别用教程的 tail transcript 法）；第一步先换带思考的模型发条测试，确认思考正文真能拿到，再动前端。
3. **「多模型群聊」——从否决改为可做，走 API 路线**。不碰教程的 CLI OAuth 套路（ToS 风险随之消失），直接用前端现成的多供应商体系接各家 API。Emet 单一人格定位不变，群聊做成独立的「圆桌」玩法。注意：群聊消耗的是各家 API 余额（按量）。
4. **⑥ Cloudflare Tunnel——静怡态度转变**。信箱模式（手机中转 relay）做完后她自己的结论是「还不如暴露公网」。此项从「观望」升为「待她正式点头即可按项目书第一期开工」。
5. 附带动作：静怡不想本机会话记录被自动清除，已在 ~/.claude/settings.json 设 `cleanupPeriodDays: 3650`（原默认 30 天；删掉这行即恢复）。
...
## 二、最优先：先查清一个涉钱疑问（编号①）【已解除，见顶部勘误 1】
...
## 五、看过但明确不做的三件（含否决理由，防止将来重复调研）【第 1、3 条已被顶部勘误 2、3 翻案】
...
## 六、建议顺序（2026-07-15 按勘误更新）

**桥补思考显示（静怡指定的下一步）→ ②③④⑤ 随时可做 → ⑥ Tunnel 待正式点头 → 多模型群聊（API 圆桌，待排期）→ ⑦ 远期观察。**

其中 ④ 建议排在 ⑤ 前（⑤ 的挂住风险需要 ④ 兜底）；③ 和 ④ 可以同一次改。①计费核查已撤销。
